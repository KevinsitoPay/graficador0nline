...
def generate_text_report(report):
    ...
    lines.append("-" * 60)
    lines.append(f"{'Fixture':<25} {'Comp':>5} {'Brackets':>8} {'Pair%':>7} {'Qual%':>7} {'Status':>8}")
    lines.append("-" * 60)
    ...
    for f in report["fixtures"]:
        ...
        lines.append(f"{name:<25} {comp:>5} {brackets:>8} {pair:>7} {qual:>7} {status:>8}")
...
def main():
    ...
    # Save text
    text_path = output_dir / f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
